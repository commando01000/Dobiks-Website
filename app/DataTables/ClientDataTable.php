    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('action', function (client $client) {
                return view('back/Clients.action', compact('client'));
            })
            ->editColumn("images", function (client $client) {
                if ($client->images) {
                    if (Storage::exists($client->images)) {
                        $return = "<img src='" . Storage::url($client->images) . "' width='50' />";
                    } else {
...
//        return $model->newQuery()
//            ->select(['clients.*', 'client_categories.name as category_name'])
//            ->join('client_categories', 'client_categories.id', '=', 'clients.category_id')
//            ->orderBy('id', 'asc');
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        // $canCreateclient = \Auth::user()->can('create-blog');
        // $canExportclient = \Auth::user()->can('export-blog');
        $buttonsConfig = [];
        // if ($canCreateclient) {
            $buttonsConfig[] =  [
                'extend' => 'create',
                'className' => 'btn btn-light-primary no-corner me-1 add_module',
                'action' => "function ( e, dt, node, config ) { window.location = '" . route('client.create') . "'; }",
            ];
        // }
        $exportButtonConfig = [];
        // if ($canExportclient) {
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('cover')->title(__('cover')), 
