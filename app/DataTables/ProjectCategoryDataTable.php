...
class ProjectCategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->addColumn('action', function (ProjectCategory $category) {
                return view('back/project-category.action', compact('category'));
            })
            ->editColumn('created_at', function ($request) {
                return UtilityFacades::date_time_format($request->created_at);
            })
            ->editColumn('status', function (ProjectCategory $category) {
                $checked = ($category->status == 1) ? 'checked' : '';
                $status   = '<label class="form-switch">
                             <input class="form-check-input chnageStatus" name="custom-switch-checkbox" ' . $checked . ' data-url="' . route('projectcategory.status', $category->id) . '" type="checkbox">
                             </label>';
                return $status;
            })
        ...
        $dataTable =  $this->builder()
            ...
            if($canCreateCategory){
                $buttonsConfig[] =  [
                    ...
                    'action' => "function ( e, dt, node, config ) { window.location = '" . route('project-category.create') . "'; }",
                ];
            }
