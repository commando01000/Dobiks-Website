...
class GalleryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('action', function (Gallery $gallery) {
                return view('back/gallery.action', compact('gallery'));
            })
            ->editColumn("images", function (Gallery $gallery) {
                if ($gallery->images) {
                    if (Storage::exists($gallery->images)) {
                        $return = "<img src='" . Storage::url($gallery->images) . "' width='50' />";
                    } else {
    ...
    public function query(Gallery $model): QueryBuilder
    {
        ...
        return $model->newQuery()->select(['gallerys.*', 'gallery_categories.name as category_name'])
            ->join('gallery_categories', 'gallery_categories.id', '=', 'gallerys.category_id')->orderBy('id', 'asc');
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        $canCreateGallery = \Auth::user()->can('create-gallery');
        $canExportGallery = \Auth::user()->can('export-gallery');
        $buttonsConfig = [];
        if ($canCreateGallery) {
            $buttonsConfig[] =  [
                'extend' => 'create',
                'className' => 'btn btn-light-primary no-corner me-1 add_module',
                'action' => "function ( e, dt, node, config ) { window.location = '" . route('gallery.create') . "'; }",
            ];
        }
        $exportButtonConfig = [];
        if ($canExportGallery) {
